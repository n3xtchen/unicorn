...
`wikilink-missing` 按 Obsidian 的真实规则解析：全路径（带/不带 `.md`）、文件名；且 `[[#标题]]`、`[[#^块]]` 是合法的同文档引用，不报警。**frontmatter `aliases` 不算命中** —— 实测 Obsidian 不把别名当链接目标（D34），所以 `[[别名]]` 会被如实报成死链。链接内部的内容不参与拼写检查（否则 `[[02-Done/…]]` 里的 `Done` 会被当成大小写错误）。
...
- **目标是真实文件**：按 Obsidian 的解析规则（全路径带/不带 `.md`、文件名）解析不到就退出 7。**别名不算**：`aliases` 只是搜索与 `[[` 补全的助手，不是可解析的链接目标（D34），拿它当链接就是一条死链。（别跟**显示别名** `[[笔记\|别名]]` 混 —— 那是渲染层的，目标看 `\|` 左边，不受这条影响。）
...
链接的另一头是块的**块锚点**（`^<块 id>`），新捕获的块自动就有，**钉在正文第一行**。

**id 必须用落盘后的那个。** 块 id 是 `<YYYYMMDD>-<HHmm>-<sha1(正文) 前 4 位>`，`HHmm` 取的是**落盘时刻** ——
所以**每次 dry-run 给出的 id 都不一样**，那只是彩排值。顺序只能是：**先把思考块写下去**，从 `--write` 的输出里拿真实 id，
**再**用它造待办行、跑 `--kind=todo`。反着做（拿 dry-run 的 id 预填）会写出一个指向空处的链接，而且**一路全绿**：
行本身合法、待办通道五项校验全过、`--verify-ids` 不报（它只核块内等式），Obsidian 也不报（它只看那篇笔记在不在，块锚点存不存在不查）——
**没有任何一道检查会发现它是死的**。已经写错了就走 `--repair-text` 改，
而且**要整条 `[[…#^id|↩]]` 一起替换、别拿裸 id**（裸 id 的右值在原文里本就出现过，回代守卫会拦）。

**要指到具体哪一句：给那一行补行锚点。** 块锚点代表的是**整条思考**（钉在首行）。
`↩` 想跳到某一句本身时，给那一行再补一个**行锚点** `^<块 id>-<行号>`（乙案：**只在点名时补**，不批量）：

```bash
# 先看这一块有哪几行正文、每行补上会叫什么、现成链接长什么样（不写盘）
scripts/journal_apply.mjs --add-anchors --id=20260923-0923-0411

# 选中第 2 行（先 dry-run，确认后加 --write）
scripts/journal_apply.mjs --add-anchors --id=20260923-0923-0411 --line=2
scripts/journal_apply.mjs --add-anchors --id=20260923-0923-0411 --line=2 --write
```

然后待办里写 `[[2026-09-39w-23#^20260923-0923-0411-2|↩]]`（落盘那一次会把现成片段直接打出来，照抄）。

- `--line` 数是**块内正文的行号**（1 基），不是文件行号。不给 `--line` 就先列出来给你选。
- **按需才补**：哪一行值得指是语义判断，脚本猜不出来；全篇每行钉一个只会把源码视图糊掉。
- 行锚点 id 里**没有时刻成分**，落盘一次就定死（不像块 id 那样随 dry-run 时刻变）—— 所以「先补锚点、再拿它写待办」是稳的顺序，"先拿 dry-run 的 id" 那个坑在这里不存在。
- 块 id 一变（正文被改），`--fix-written` / `--verify-ids --write` 会把块内**每个**锚点的前缀一起换掉；行号漂了（中间插过/删过行）会重编号并报警，引用它的链接得人工核对。
- 钉不了的会说明原因（空行、表格行、代码块里、注释行、分隔线）；块锚点该在的那一行（首行）如果还没钉，就直接钉裸块 id，一举两得。
- **旧笔记不回填**：老块的块锚点照旧（在末行的照样能用、`--verify-ids` 照样认），行锚点只在真用到时补。
- **老块想搬到首行**：`--add-anchors --id=<块 id> --move`（纯搬移：正文一个字节不动、id 不变，索引行与待办里的链接都不用改）。不是必须 —— 末行只是 hover 预览短一截。
...
**块锚点与行锚点。** 块锚点 `^<块 id>` 钉在正文**第一行**，代表**整条思考**；行锚点 `^<块 id>-<行号>` 钉在块内某一行，代表**那句话本身**（按需才补，见「标出来源」那节）。
于是 `[[2026-09-38w-20#^20260920-1037-8dc5]]` 跳到那段思考，`[[2026-09-39w-23#^20260923-0923-0411-2|↩]]` 直接跳到第 2 行那一句。

**为什么块锚点钉首行（2026-09-23 改的口径）。** Obsidian 给块的**范围**只有锚点所在那一行（实测：`blocks[id].position` 始终只覆盖那一行），但**渲染层会把带子项的列表项连同子项一起渲染**。捕获往往整条挂在一条顶层列表项下 ——
所以锚点在首行时，hover 预览 / `![[…]]` 嵌入给的是**整条**；在末行（旧口径）只剩最深的那条叶子。两种位置都注册、都能跳，差的就是「预览多大」。
首行钉不住时（首行是标题 / 表格 / 围栏 / 注释 / 分隔线）**自动退末行**；两处都不行就不钉，把原因报出来（正文照写）。

Obsidian 只给落在一段**真文本**里的锚点注册块 —— 整行只有 `^id`、或跟在别的字后面都行，**HTML 注释行不算**（实测：写在 `<!-- -->` 里的 `[[…]]` 不进链接表，点不动）。所以锚点不能挂在 `jc:end` 上，只能挤进正文某一行。

**锚点不算正文。** 算 sha1、比对幂等之前一律**逐行**剥掉（一块里可能挂着好几个行锚点）：不剥，锚点里的 id 会自指进 hash，而同一段思考第二次捕获也会被当成新块重复写。剥法按形状认（`<8位日期>-<4位时刻>-<4位 hash>[-行号]`），正文里碰巧以空格加 `^词` 结尾的句子不会被误剥。`--verify-ids` 因此报三件事 —— 正文 hash 对不对、每个锚点的前缀是不是本块 id、锚点位置对不对（行锚点的行号是不是等于它**实际所在的行号**；块锚点必须在**首行或末行**，中间就是错的）；任一不对，`--write` 一起修回来（`jc:begin` / `jc:end` / 块内每个锚点 / 派生层索引行的块链接；行号漂了就按实际行号重编号并在报告里点名）。**旧块没有锚点照样通过**，末行锚点的旧块也照样通过（不回填、不搬移），脚本也不会借修 id 的机会替它补上。
...
`## 今日待办` 里用户自己的 `` [[…#^id|↩]] `` 不被牵连 —— 那是另一层、另一件事（已知缺口：待办出处链接不会跟着重算，暂不自动改；行锚点重编号时会在报告里把「哪些链接要人工核对」点名出来）。

另有一个更静的缺口：链接指向**不存在的锚点**时，脚本不管，Obsidian 也不报 —— 它只看到那篇笔记在，`resolvedLinks` 就登记了（实测：`[[笔记#^不存在的 id]]` 不进 `unresolvedLinks`）。所以「拿落盘后的锚点 id 再造链接」这条纪律目前是唯一一道拦，见「标出来源」。
...
块必须**已经有块锚点** —— 没锚点链接就跳不过去。这里**不代劳补锚点**，只报 `anchor-missing`，叫你先跑 `--add-anchors`（两件事分开，各自可核）。
...
# 给旧块补上块锚点（纯追加，钉在正文首行；dry-run）
scripts/journal_apply.mjs --add-anchors

# 把某一块的块锚点从末行搬到首行（纯搬移：正文不动、id 不变、链接照旧；dry-run）
scripts/journal_apply.mjs --add-anchors --id=20260923-0923-0411 --move

# 只给某一块的某一行补行锚点（乙案：按需才补）
scripts/journal_apply.mjs --add-anchors --id=20260923-0923-0411              # 先列出行号与现成链接
scripts/journal_apply.mjs --add-anchors --id=20260923-0923-0411 --line=2    # dry-run
scripts/journal_apply.mjs --add-anchors --id=20260923-0923-0411 --line=2 --write
...
# 把派生层裹反引号的块 id 迁移成块链接（dry-run；块缺块锚点的先跑 --add-anchors）
...
| `anchor-missing` | 块没有块锚点，链接跳不过去。先跑 `--add-anchors`，再重跑 `--link-block-ids`。 |
