...
任何笔记里的任务行都会被 obsidian-tasks 的查询扫到（插件配置 `globalQuery: path does not include 998-template`），所以在项目笔记、调研笔记里写 `- [ ]` 也按这套走：

- **行尾只能是一个 emoji 字段**（`➕` / `🛫` / `⏳` / `📅` / `🔁` / `✅`）。字段正则锚定行尾、从右往左反复剥，行尾挂上 `】`、`-->`、逗号，**全部**日期字段就都解析不出来（库里有现存中招行）。
- **字段顺序固定**：描述 → 行内标签 → 优先级 → `➕` → `🛫` → `⏳` → `📅` → `🔁` → `✅`。乱序会让状态分组与日期查询漏行。
- **一行一个 checkbox**：顶格写 `- [ ]`，子项缩进 4 空格（与本 skill 第 6 条一致），同一个块内不混 Tab。
- 手写的任务行要自己带 `➕ YYYY-MM-DD`，不写就进不了 `(created …)` 这类查询。

自检（markdownlint 不管这块，这一步只能自己看）：

```bash
rg -n '^\s*- \[[ x?/-]\]' <file.md>    # 把任务行列出来，逐行确认行尾是 emoji 字段、顺序对得上
```

契约正文（状态词表、闸门、出处链接的位置）在 [daily-journal 的 vault-conventions.md](../daily-journal/references/vault-conventions.md)：那是唯一权威，这里只放写 .md 时用得上的判据与指针。
...
本 skill 也管**正文表述**，不限 Obsidian、不限 vault：任何仓库、任何目录下写出的中文正文都适用。格式之外再过一遍：

- 不写计数式开篇（「本周三件事」「三条主线」），首句直接给判断或状态；
- 不用「不在……而是……」「不是……而是……」对偶句，拆成陈述句；
- 不写「回答了三个问题……」这类问答式总结，改写成「做到了什么、没做到什么」；
- 连续段落不要用同一句型开头；
- 删掉写作动机与元话语（「目的：」「（可分享版）」「本节介绍……」）；
- 收尾不用「综上」「小结」复述前文。
...
Read [de-ai-writing.md](references/de-ai-writing.md) for the six patterns with before/after examples and the full rule set.
